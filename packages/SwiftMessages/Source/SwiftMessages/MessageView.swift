...
/*
 */
open class MessageView: BaseView, @preconcurrency Identifiable, @preconcurrency AccessibleMessage, @preconcurrency HapticMessage {
    ...
    open override func didMoveToWindow() {
        ...
        self.addDropShadow(
            color: (backgroundView.backgroundColor ?? .black).withAlphaComponent(0.4),
...
extension UIView {
    func addDropShadow(color: UIColor = .black, opacity: Float = 0.5, offSet: CGSize = .zero, radius: CGFloat = 5, scale: Bool = true) {
        layer.masksToBounds = false
        layer.shadowColor = color.cgColor
        layer.shadowOpacity = opacity
        layer.shadowOffset = offSet
        layer.shadowRadius = radius
        // If you need to optimize performance for complex shadow paths, you can uncomment the line below.
